...
public class AliasCommand extends Command {
    ...
    public AliasCommand(KibbleCommands plugin, AliasDefinition definition) {
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        if (definition.hasPermission() && !sender.hasPermission(definition.getPermission())) {
            sendMsg(sender, plugin.prefix() + definition.getPermissionMessage());
            return true;
        }
        if (definition.isPlayerOnly() && !(sender instanceof Player)) {
            sendMsg(sender, plugin.prefix() + "&cThis command can only be used by players.");
            return true;
        }
        if (definition.isConsoleOnly() && sender instanceof Player) {
            sendMsg(sender, plugin.prefix() + "&cThis command can only be used from the console.");
            return true;
        }
        if (plugin.getConfig().getBoolean("require-use-permission", false) && !sender.hasPermission("kibblecommands.use")) {
            sendMsg(sender, plugin.prefix() + "&cYou do not have permission to use alias commands.");
            return true;
        }
        if (definition.getCooldown() > 0 && sender instanceof Player player) {
            long remaining = plugin.getCooldownTracker().getRemainingCooldown(definition.getAlias(), player.getUniqueId(), definition.getCooldown());
            if (remaining > 0) {
                sendMsg(sender, plugin.prefix() + "&cYou must wait &e" + remaining + "s &cbefore using this command again.");
                return true;
            }
            plugin.getCooldownTracker().recordUse(definition.getAlias(), player.getUniqueId());
        }
        String targetCommand = buildTargetCommand(sender, commandLabel, args);
        if (targetCommand.isBlank()) {
            sendMsg(sender, plugin.prefix() + "&cThis alias has no command configured.");
            return true;
        }
        if (plugin.getConfig().getBoolean("notify-on-alias-use", false)) {
            ...
            String formatted = message.replace("{alias}", commandLabel).replace("{target}", targetCommand);
            sendMsg(sender, plugin.prefix() + formatted);
        }
        ...
        try {
            ...
            sendMsg(sender, plugin.prefix() + "&cFailed to execute the aliased command. Check console.");
    ...
    public List<String> tabComplete(CommandSender sender, String alias, String[] args) {
        try {
            String target = KibbleCommands.normalizeTargetCommand(definition.getCommand());
            ...
            if (targetCommand == null || targetCommand == this) return List.of();
    ...
    private String buildTargetCommand(CommandSender sender, String commandLabel, String[] args) {
        String configuredCommand = KibbleCommands.normalizeTargetCommand(definition.getCommand());
